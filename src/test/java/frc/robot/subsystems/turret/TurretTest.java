...
//        Assert.assertEquals(turnLocalization.calculateTargetAngle(new Pose2d(3.5, 0.2, new Rotation2d(Math.toRadians(-20)))), 42.932, 0.1);
        ...
        // Outer port tests
        Assert.assertEquals(VisionModule.calculateTargetAngle(new Pose2d(7, 4, new Rotation2d(Math.toRadians(56.4))), false), -45, 0.1);
        Assert.assertEquals(VisionModule.calculateTargetAngle(new Pose2d(3.5, 0.2, new Rotation2d(Math.toRadians(29.4))), false), -5.195, 0.1);
        Assert.assertEquals(VisionModule.calculateTargetAngle(new Pose2d(3, 0, new Rotation2d(Math.toRadians(20.7))), false), 3.4138, 0.1);
        Assert.assertEquals(VisionModule.calculateTargetAngle(new Pose2d(3.5, 0.2, new Rotation2d(Math.toRadians(-20))), false), 44.204, 0.1);
    }
}